...
export function OnlineNow({
	...
	const deviceRows = devices?.length ? devices : defaultDevices;
	const count = liveCount ?? 94;
	const deltaValue = delta ?? 14.8;

	return (
		<Card className="gap-0 pb-0 md:col-span-2 lg:col-span-1 dark:bg-transparent">
			<CardHeader className="flex flex-row items-start justify-between gap-3 border-b">
				<div className="flex min-w-0 flex-col gap-0">
					<CardTitle className="font-mono text-2xl tabular-nums">{count}</CardTitle>
					<CardDescription>
						<Tooltip>
				...
				<Delta value={deltaValue} variant="badge">
